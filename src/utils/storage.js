...
export const updateAutoGenerated = async (companyRef, autoGeneratedData) => {
    try {
        let companyId;
        ...
        // Update company workflow status to indicate analysis is in progress
        const { error: updateError } = await supabase
            .from('companies')
            .update({ workflow_status: 'analysis_in_progress' })
            .eq('id', companyId);
